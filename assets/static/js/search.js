...
class SearchResult {
    ...
    display(map, preview) {
        ...
        this.inner.forEach((item) => {
            ...
            link.addEventListener("mouseenter", () => this.displayPreview(filename, map[filename]["title"], map[filename]["description"], map[filename]["tags"], preview))
...
class ContentMap {
    ...
    evaluateString(string, keyword) {
        return new LevenshteinDistance(string, keyword)
            .normalize()
            .inner;
    }
    ...
    evaluateArray(array, keyword) {
        return array.reduce((max, item) =>
            Math.max(max, this.evaluateString(item, keyword)), 0
        );
    }
